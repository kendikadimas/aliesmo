<template>
    <div>
        <section class="min-h-[100dvh] grid lg:grid-cols-2">
            <div class="relative flex items-center px-4 sm:px-8 lg:px-12 xl:px-16 py-24 lg:py-0 bg-ivory">
                <div class="max-w-lg">
                    <div class="flex items-center gap-3 mb-8">
                        <span class="w-8 h-[1px] bg-bronze"></span>
                        <span class="text-[11px] tracking-[0.25em] uppercase text-bronze">Koleksi Premium 2025</span>
                    </div>
                    <h1 class="text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-light leading-[0.95] tracking-tight text-charcoal text-balance">
                        Kemeja <span class="font-medium">Premium</span><br>
                        untuk Pria<br>
                        <span class="italic font-serif font-light text-bronze">Modern</span>
                    </h1>
                    <p class="mt-6 text-base lg:text-lg text-charcoal/55 leading-relaxed max-w-md">
                    ...
                    <div class="mt-10 flex items-center gap-8 text-sm text-charcoal/40">
                        <div class="flex items-center gap-2">
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.2"><path d="M12 22s-8-4.5-8-11.8A8 8 0 0112 2a8 8 0 018 8.2c0 7.3-8 11.8-8 11.8z"/><circle cx="12" cy="10" r="3"/></svg>
                            <span>JNE / J&T / SiCepat</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>
                            <span>Bayar di Tempat</span>
                        </div>
                    </div>
                </div>
            </div>
            <div class="relative min-h-[50vh] lg:min-h-screen bg-aliesmo-100 overflow-hidden">
                <div class="absolute inset-0 bg-gradient-to-t lg:bg-gradient-to-l from-ivory/40 via-transparent to-transparent z-10"></div>
                <div class="absolute inset-0 flex items-center justify-center p-8 lg:p-12">
                    <div class="relative w-full max-w-md">
                        <div class="aspect-[3/4] relative overflow-hidden">
                            <img src="https://picsum.photos/seed/kemeja-putih-formal/800/1067" alt="Aliesmo Premium Shirt" class="w-full h-full object-cover" />
                            <div class="absolute inset-0 bg-gradient-to-t from-charcoal/20 to-transparent"></div>
                        </div>
                        <div class="absolute -bottom-4 -right-4 lg:-bottom-6 lg:-right-6 bg-ivory p-4 lg:p-6 shadow-xl">
                            <p class="text-xs tracking-[0.2em] uppercase text-charcoal/50">Mulai dari</p>
                            <p class="text-xl lg:text-2xl font-medium text-charcoal">Rp 149.000</p>
                        </div>
                    </div>
                </div>
                <div class="absolute bottom-6 left-6 z-20 hidden lg:block">
                    <p class="text-[10px] tracking-[0.3em] uppercase text-charcoal/30">New Collection</p>
                </div>
            </div>
        </section>
        ...
        <section class="py-12 lg:py-16 bg-white overflow-hidden">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex items-center gap-6 mb-8">
                    <span class="text-[11px] tracking-[0.25em] uppercase text-charcoal/40 shrink-0">Kategori</span>
                    ...
                    <div @click="filters.category = ''; fetchProducts()" class="snap-start shrink-0 cursor-pointer group">
                        <div class="w-28 h-36 lg:w-32 lg:h-40 bg-ivory border border-dashed border-aliesmo-200/70 overflow-hidden flex items-center justify-center group-hover:border-charcoal/30 transition-colors">
                            <div class="text-center">
                                ...
                                <p class="text-xs text-charcoal/40 mt-2 group-hover:text-charcoal/60 transition-colors">Semua</p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <section id="shop" class="py-16 lg:py-24">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex items-end justify-between mb-10 lg:mb-12">
                    <div>
                        <div class="flex items-center gap-3 mb-3">
                            <span class="w-6 h-[1px] bg-bronze"></span>
                            <span class="text-[11px] tracking-[0.25em] uppercase text-bronze">Produk</span>
                        </div>
                        <h2 class="text-3xl lg:text-4xl font-light text-charcoal">Koleksi <span class="font-medium">Terbaru</span></h2>
                    </div>
                ...
                <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 lg:gap-8">
                    <div v-for="product in products" :key="product.id" class="group cursor-pointer" @click="$router.push(`/products/${product.slug}`)">
                        <div class="aspect-[3/4] bg-aliesmo-100 overflow-hidden relative">
                            ...
                            <div v-if="product.stock === 0" class="absolute inset-0 bg-charcoal/60 flex items-center justify-center">
                                <span class="text-white text-xs tracking-[0.2em] uppercase">Stok Habis</span>
                            </div>
                        ...
                        <div class="mt-4 space-y-1.5">
                            <p class="text-[11px] tracking-[0.2em] uppercase text-bronze/60">{{ product.category?.name || 'Kemeja' }}</p>
                            <h3 class="text-sm font-medium text-charcoal group-hover:text-bronze transition-colors leading-snug">{{ product.name }}</h3>
                            <div class="flex items-center justify-between">
                                <p class="text-sm font-medium text-charcoal">Rp {{ formatPrice(product.price) }}</p>
                                <div class="flex items-center gap-0.5">
                                    <svg v-for="n in 5" :key="n" class="w-3 h-3" :class="n <= 4 ? 'text-bronze' : 'text-aliesmo-300/50'" viewBox="0 0 24 24" fill="currentColor"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div v-if="hasMore && products.length" class="text-center mt-12 lg:mt-16">
                    <button @click="loadMore" class="px-10 py-3.5 border border-charcoal/20 text-xs tracking-[0.2em] uppercase text-charcoal/60 hover:bg-charcoal hover:text-ivory transition-all active:scale-[0.98]">
                        Muat Lainnya
                    </button>
                </div>
            </div>
        ...
        <section class="py-16 lg:py-20 bg-charcoal text-ivory overflow-hidden">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="grid lg:grid-cols-3 gap-8 lg:gap-12">
                    <div class="lg:col-span-1">
                        <div class="flex items-center gap-3 mb-4">
                            <span class="w-6 h-[1px] bg-bronze/60"></span>
                            <span class="text-[11px] tracking-[0.25em] uppercase text-bronze/60">Mengapa Aliesmo</span>
                        </div>
                        <h2 class="text-3xl lg:text-4xl font-light leading-tight text-balance">
                            Setiap Detail<br>
                            <span class="font-medium">Diperhitungkan</span>
                        </h2>
                    </div>
                    <div class="lg:col-span-2 grid sm:grid-cols-2 gap-6 lg:gap-8">
                        <div class="border-t border-ivory/10 pt-6">
                            <span class="text-3xl font-light text-bronze">01</span>
                            <h3 class="mt-4 text-base font-medium text-ivory">Katun Premium</h3>
                            <p class="mt-2 text-sm text-ivory/50 leading-relaxed">Kain katun pilihan dengan thread count tinggi, memberikan kenyamanan maksimal sepanjang hari.</p>
                        </div>
                        <div class="border-t border-ivory/10 pt-6">
                            <span class="text-3xl font-light text-bronze">02</span>
                            <h3 class="mt-4 text-base font-medium text-ivory">Jahitan Presisi</h3>
                            <p class="mt-2 text-sm text-ivory/50 leading-relaxed">Dijahit oleh pengrajin berpengalaman dengan teknik fused collar untuk bentuk yang tetap sempurna.</p>
                        </div>
                        <div class="border-t border-ivory/10 pt-6">
                            <span class="text-3xl font-light text-bronze">03</span>
                            <h3 class="mt-4 text-base font-medium text-ivory">Desain Klasik</h3>
                            <p class="mt-2 text-sm text-ivory/50 leading-relaxed">Siluet timeless yang tetap relevan di setiap musim. Dari formal hingga casual, semua tersedia.</p>
                        </div>
                        <div class="border-t border-ivory/10 pt-6">
                            <span class="text-3xl font-light text-bronze">04</span>
                            <h3 class="mt-4 text-base font-medium text-ivory">Garansi Kualitas</h3>
                            <p class="mt-2 text-sm text-ivory/50 leading-relaxed">Setiap kemeja melewati quality control ketat. Tidak puas? Garansi 30 hari uang kembali.</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
                    <div class="relative order-2 lg:order-1">
                        <div class="aspect-[4/5] overflow-hidden">
                            <img src="https://picsum.photos/seed/kemeja-pengrajin/700/875" alt="Aliesmo Craftsmanship" class="w-full h-full object-cover" />
                        </div>
                        <div class="absolute -bottom-4 -left-4 lg:-bottom-6 lg:-left-6 bg-ivory p-4 lg:p-6 shadow-xl max-w-[200px]">
                            <p class="text-xs tracking-[0.2em] uppercase text-charcoal/50">Pengrajin</p>
                            <p class="text-lg font-medium text-charcoal">Berpengalaman</p>
                            <p class="text-sm text-charcoal/60 mt-1">+10 tahun</p>
                        </div>
                    </div>
                    <div class="order-1 lg:order-2">
                        <div class="flex items-center gap-3 mb-3">
                            <span class="w-6 h-[1px] bg-bronze"></span>
                            <span class="text-[11px] tracking-[0.25em] uppercase text-bronze">Cerita Kami</span>
                        </div>
                        <h2 class="text-3xl lg:text-4xl font-light text-charcoal">Kerajinan dalam <span class="font-medium">Setiap Detail</span></h2>
                        <div class="mt-6 space-y-4 text-base text-charcoal/55 leading-relaxed max-w-lg">
        ...
        <section class="py-16 lg:py-20 bg-aliesmo-100/50">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="max-w-xl mx-auto text-center">
                    <div class="flex items-center justify-center gap-3 mb-3">
                        <span class="w-6 h-[1px] bg-bronze"></span>
                        <span class="text-[11px] tracking-[0.25em] uppercase text-bronze">Newsletter</span>
                        <span class="w-6 h-[1px] bg-bronze"></span>
                    </div>
                    <h2 class="text-3xl lg:text-4xl font-light text-charcoal text-balance">Dapatkan <span class="font-medium">Info Terbaru</span></h2>
                    <p class="mt-4 text-sm text-charcoal/55">Daftar untuk mendapatkan notifikasi koleksi baru, promo eksklusif, dan inspirasi gaya.</p>
                    <form @submit.prevent="handleSubscribe" class="mt-8 flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
                        <input v-model="email" type="email" required placeholder="Masukkan email Anda" class="flex-1 border border-aliesmo-200/70 px-4 py-3 text-sm text-charcoal placeholder:text-charcoal/30 focus:border-charcoal/50 focus:outline-none transition-colors bg-white">
                        <button type="submit" class="px-8 py-3 bg-charcoal text-ivory text-xs tracking-[0.2em] uppercase hover:bg-charcoal/90 transition-all active:scale-[0.98] whitespace-nowrap">
                            Daftar
                        </button>
                    </form>
                    <p v-if="subscribed" class="mt-4 text-sm text-green-700">Terima kasih! Anda telah terdaftar.</p>
                </div>
            </div>
        </section>
    </div>
