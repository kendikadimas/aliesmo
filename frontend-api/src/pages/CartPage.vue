<template>
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
        ...
        <div v-else class="mt-8 lg:mt-10">
            <div class="space-y-4">
                <div v-for="item in items" :key="item.product_id" class="flex items-center gap-4 lg:gap-6 bg-white p-4 lg:p-6 border border-ink-10">
                    <div class="w-16 h-20 lg:w-20 lg:h-24 shrink-0 bg-ink-05 overflow-hidden">
                        <img v-if="item.thumbnail" :src="item.thumbnail" :alt="item.name" class="w-full h-full object-cover" />
                        <div v-else class="w-full h-full flex items-center justify-center text-ink-20 text-lg">A</div>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                        <button @click="decrease(item.product_id)" class="w-8 h-8 border border-ink-10 flex items-center justify-center text-sm hover:border-charcoal/30 transition-colors active:scale-95">−</button>
                        <span class="w-8 text-center text-sm font-medium">{{ item.quantity }}</span>
                        <button @click="updateQuantity(item.product_id, item.quantity + 1)" class="w-8 h-8 border border-ink-10 flex items-center justify-center text-sm hover:border-charcoal/30 transition-colors active:scale-95">+</button>
                    </div>
            ...
            <div class="mt-8 pt-8 border-t border-ink-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <p class="text-sm text-charcoal/60">Total</p>
                    <p class="text-3xl font-light text-charcoal">Rp {{ formatPrice(total()) }}</p>
                </div>
                <div class="flex gap-3">
                    <router-link to="/" class="px-6 py-3 border border-charcoal/20 text-sm tracking-widest uppercase text-charcoal/70 hover:bg-charcoal hover:text-paper transition-all active:scale-[0.98]">
                        Lanjut Belanja
                    </router-link>
                    <router-link to="/checkout" class="px-8 py-3 bg-charcoal text-paper text-sm tracking-widest uppercase hover:bg-charcoal/90 transition-all active:scale-[0.98]">
                        Checkout
                    </router-link>
                </div>
            </div>
