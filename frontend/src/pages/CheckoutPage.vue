        ...
        <form v-else @submit.prevent="submitOrder" class="mt-8 lg:mt-10 grid lg:grid-cols-5 gap-8 lg:gap-12">
            <div class="lg:col-span-3 space-y-6">
                ...
                <div class="bg-white p-6 lg:p-8 rounded-2xl border-2 border-maroon-50">
                    ...
                    <div>
                        ...
                        <textarea v-model="form.shipping_address" required placeholder="Contoh: Jl. Merdeka No. 123, RT 01/RW 02, Kelurahan, Kecamatan, Kota, Provinsi" rows="3" class="w-full border-2 border-maroon-100 rounded-xl px-4 py-2.5 text-sm text-charcoal placeholder:text-charcoal/30 focus:border-maroon focus:outline-none transition-colors resize-none"></textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-2">
                <div class="bg-white p-6 lg:p-8 rounded-2xl border-2 border-maroon-50 lg:sticky lg:top-28">
                    ...
                    <div class="border-t-2 border-maroon-100 mt-4 pt-4 space-y-2">
                        ...
                        <div class="flex justify-between text-lg font-bold text-charcoal pt-3 border-t-2 border-maroon-100">
                            <span>Total</span>
                            <span class="text-maroon">Rp{{ formatPrice(total()) }}</span>
                        </div>
                    </div>

                    <div v-if="error" class="mt-4 p-3 bg-red-50 rounded-xl border border-red-200 text-red-700 text-sm">{{ error }}</div>

                    <button type="submit" class="w-full mt-6 px-8 py-3.5 bg-maroon text-white text-sm font-semibold rounded-xl hover:bg-maroon-600 transition-all active:scale-[0.97] shadow-lg shadow-maroon/25">
                        Pesan Sekarang
                    </button>

                    <p class="mt-4 text-xs text-center text-charcoal/40 leading-relaxed">Dengan pesan, kamu setuju sama <a href="#" class="text-maroon hover:text-maroon-600 underline">syarat & ketentuan</a> kita.</p>
                </div>
            </div>
        </form>
    </div>
