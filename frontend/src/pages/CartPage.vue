<template>
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
        ...
        <div v-else class="mt-8 lg:mt-10">
            <div class="space-y-3">
                <div v-for="item in items" :key="item.product_id" class="flex items-center gap-4 bg-white p-4 lg:p-5 rounded-2xl border-2 border-maroon-50 hover:border-maroon-100 transition-all">
                    <div class="w-16 h-20 lg:w-20 lg:h-24 shrink-0 bg-maroon-50 rounded-xl overflow-hidden">
                    ...
                    <div class="flex items-center gap-2">
                        <button @click="decrease(item.product_id)" class="w-8 h-8 rounded-lg border-2 border-maroon-200 flex items-center justify-center text-sm font-bold text-charcoal hover:border-maroon hover:text-maroon transition-colors active:scale-95">−</button>
                        <span class="w-8 text-center text-sm font-bold">{{ item.quantity }}</span>
                        <button @click="updateQuantity(item.product_id, item.quantity + 1)" class="w-8 h-8 rounded-lg border-2 border-maroon-200 flex items-center justify-center text-sm font-bold text-charcoal hover:border-maroon hover:text-maroon transition-colors active:scale-95">+</button>
                    </div>
            ...
            <div class="mt-8 pt-6 border-t-2 border-maroon-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <p class="text-sm text-charcoal/50">Total Belanja</p>
                    <p class="text-3xl font-bold text-maroon">Rp{{ formatPrice(total()) }}</p>
                </div>
                <div class="flex gap-3">
                    <router-link to="/" class="px-6 py-3 bg-white text-charcoal text-sm font-semibold rounded-xl border-2 border-maroon-200 hover:border-maroon hover:bg-maroon-50 transition-all active:scale-[0.97]">
                        Lanjut Belanja
                    </router-link>
