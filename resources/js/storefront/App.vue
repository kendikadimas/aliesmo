<template>
    <div class="min-h-screen bg-putih">
                ...
                <div class="flex items-center justify-between h-16 lg:h-[72px]">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <router-link to="/" class="flex items-center">
                            <span class="text-xl sm:text-2xl lg:text-3xl font-extrabold tracking-[0.22em] text-charcoal font-serif hover:text-maroon transition-colors">ALIESMO</span>
                        </router-link>
                    </div>
                ...
                <div class="max-w-7xl mx-auto px-8">
                    <nav class="flex items-center justify-center gap-8 h-10">
                        <router-link
                            ...
                            :class="{ 'border-b-2 border-maroon text-maroon font-bold': !$route.query.category && $route.path === '/' }"
                        >
                            Semua
                        </router-link>
                        <router-link
                            ...
                            class="text-xs font-bold tracking-wider text-charcoal/70 hover:text-maroon hover:border-b-2 hover:border-maroon transition-all py-2.5 uppercase"
                            :class="{ 'border-b-2 border-maroon text-maroon font-bold': $route.query.category === cat.slug }"
                        >
                            {{ cat.name }}
                        </router-link>
                    </nav>
            ...
            <div v-if="mobileOpen" class="lg:hidden border-t border-zinc-200/50 bg-putih shadow-xl fixed top-[110px] left-0 right-0 z-40 max-h-[calc(100vh-110px)] overflow-y-auto">
                <nav class="px-5 py-6 space-y-5">
                    ...
                    <div class="space-y-3 pb-4">
                        ...
                        <button v-else @click="triggerMobileLogout" class="block w-full text-center text-xs font-bold text-white bg-charcoal py-3 rounded-lg uppercase tracking-wider cursor-pointer">Keluar</button>
                    </div>
                </nav>
            </div>
        </Transition>
        ...
        <Transition name="toast">
            <div v-if="toastState.visible" class="fixed bottom-5 right-5 z-[99] max-w-sm" :class="toastState.type === 'error' ? 'bg-red-600' : 'bg-charcoal'">
                <div class="text-white text-xs px-4 py-3.5 rounded-xl shadow-2xl flex items-center gap-2.5 border border-white/10">
                    <template v-if="toastState.type === 'success'">
                        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" class="text-green-400" stroke-linecap="square"><polyline points="20 6 9 17 4 12"/></svg>
                    </template>
                    <template v-else-if="toastState.type === 'error'">
                        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" class="text-red-300" stroke-linecap="square"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                    </template>
                    <template v-else>
